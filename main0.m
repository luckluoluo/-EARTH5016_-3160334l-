%% ========================================================================
% HELMSDALE GEOTHERMAL: Pure Conduction Model
% Clean baseline simulation ignoring fluid advection. Used for comparing 
% the impact of conductive vs. convective heat transport.
% ========================================================================
...
% Initialize core domain and append buffer
...
% Property mapping for Conduction Only
...
%% 4. Initialization & Solver Loop
dt = 0.5 * (min(dx,dz)^2 / (2 * max(K_map(:)./(Rho_map(:).*Cp_map(:)))));
T = ones(Nz, Nx_core+Nx_buffer) * T_surface;
for i = 1:Nz, T(i,:) = T_surface + (i*dz)*0.03; end % Set initial gradient
